...
productsApiRouter.post(
    "/", passportCall("jwt"), checkAuthorization([ "admin" ]),
...
productsApiRouter.put(
    "/:id", passportCall("jwt"), upload.single("thumbnail"),
    checkAuthorization([ "admin" ]), async (req, res) => {
      const id =
          /^\d+$/.test(req.params.id) ? parseInt(req.params.id) : req.params.id;
      const product = req.body;
...
// DELETE
productsApiRouter.delete("/:id", passportCall("jwt"),
                         checkAuthorization([ "admin" ]), async (req, res) => {
