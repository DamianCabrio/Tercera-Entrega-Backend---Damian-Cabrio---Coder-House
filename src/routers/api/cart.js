...
cartApiRouter.get(
    "/", passportCall("jwt"), checkAuthorization([ "admin", "user" ]),
    async (_, res) => {
      const result = await cartApi.getAll();
      const cartsIds =
          result.payload ? result.payload.map((product) => product.id) : [];
      res.status(200).json(
          returnMessage(false, 200, "Carritos encontrados", cartsIds));
    });

cartApiRouter.post("/", passportCall("jwt"),
...
cartApiRouter.delete("/:id", passportCall("jwt"),
                     checkAuthorization([ "admin", "user" ]),
                     async (req, res) => {
...
cartApiRouter.post(
    "/:id/checkout", passportCall("jwt"),
    checkAuthorization([ "admin", "user" ]), async (req, res) => {
      const id =
          /^\d+$/.test(req.params.id) ? parseInt(req.params.id) : req.params.id;
      ...
      if (cart.code === 200) {
        ...
        if (products.length !== 0) {
          sendMail(config.EMAIL,
                   "Nuevo pedido de " + req.user.username + " | " +
                       req.user.email,
                   generateProductsHtml(products));
          twilioClient.messages
              .create({
                body : "Nuevo pedido de " + req.user.username + " | " +
                           req.user.email,
...
cartApiRouter.get(
    ...
    checkAuthorization([ "admin", "user" ]), async (req, res) => {
      ...
      if (cart.code !== 200) {
        ...
        res.status(cart.code).json(returnMessage(
            false, 200, "Productos encontrados", cart.payload.productos));
      }
    });
...
cartApiRouter.post(
    ...
    checkAuthorization([ "admin", "user" ]), async (req, res) => {
      const id =
          /^\d+$/.test(req.params.id) ? parseInt(req.params.id) : req.params.id;
      const productsIds =
          req.body.productos.map((id) => /^\d+$/.test(id) ? parseInt(id) : id);
...
cartApiRouter.delete("/:id/productos/:productId", passportCall("jwt"),
                     checkAuthorization([ "admin", "user" ]),
                     async (req, res) => {
                       const id = /^\d+$/.test(req.params.id)
                                      ? parseInt(req.params.id)
