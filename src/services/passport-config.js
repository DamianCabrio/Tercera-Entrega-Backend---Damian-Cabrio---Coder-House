          ...
          async (req, username, password, done) => {
            try {
              ...
              await users.createOne(newUser).then((user) => {
                if (user.code === 200) {
                  sendMail(config.EMAIL, "Nuevo Registro",
      ...
      new JWTStrategy(
          ...
          async (jwtPayload, done) => {
            try {
              ...
              return done(null,
                          returnMessage(true, 401, "Usuario no encontrado"));
            } catch (err) {
              return done(err);
            }
          }));

  passport.serializeUser((user, done) => { done(null, user.payload.id); });

  passport.deserializeUser(async (id, done) => {
    await users.getOneById(id).then((user) => { done(null, user.payload); });
  });
};
